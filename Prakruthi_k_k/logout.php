...
body { font-family:Arial,sans-serif; background:#fff0f0; padding:15px; }
...
.btn-green  { background:#c0392b; color:white; }
.btn-green:hover:not(:disabled) { background:#a93226; }
...
function doCapture(){
  ...
  ctx.fillStyle="#ffff00";ctx.font="bold 14px Arial";
  ctx.fillText("LOGOUT",10,canvas.height-oh+18);
  ctx.font="12px Arial";ctx.fillStyle="#ffff00";
  ctx.fillText('Date : '+dateStr,10,canvas.height-oh+35);
  ctx.fillText('Time : '+timeStr,10,canvas.height-oh+51);
  ctx.font='bold 10px Arial';ctx.fillStyle="#ffd700";
  ctx.fillText('Location :',10,canvas.height-oh+67);
  ctx.font='10px Arial';ctx.fillStyle="#ffd700";
...
function doSave(){
  ...
  fetch('save_logout.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:body})
  .then(function(){
    alert('Logout Saved!\n\n'+finalAddress);
